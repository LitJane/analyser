...
1.  Первичный (грязный) обучающий набор был создан в полуавтоматическом 
    режиме, для чего применялись разнообразные эвристики, fuzzy-поиск шаблонов и 
    привычные регулярные выражения. 
    Небольшая часть документом была размечена/скорректирована вручную.
    На основе первичнного набора была создана начальная модель, 
    которая дальнейшем применялась для разметки следующего 
    поколения обучающего набора. 🚧  
    Такой итеративный подход (Noisy Student) позволил
...
- analyser
 -- основной код
- analyser/vocab: данные -- словари, вспомогательные модели и и пр.
  
- bin : команды для запуска сервиса
- [cli](cli) : команды для запуска обучения, публикации модели, пр.
- experiments: TODO
- colab_support: утилиты для отображения данных в Jupyter Notebook
- gpn: константы, связвнные с организацией
- integration: интерфейсы интеграции со сторонними системами
- tests: юнит-тесты 
- tf_support: фрейморк-зависимые утилиты для TensorFlow
- training_reports: отчеты о процессе обуччения, структуре обучающего набора и тп.
- trainsets: TODO
- work: промежуточные файлы, необходимые в фазе обучения 
___
