...
4. Производится эмбеддинг текста -- перевод токенов в векторное представление. 
   Для генерации "эмбеддингов" мы используем предварительно обученную DeepPavlov модель ELMO.
...
7. На выходе из сети -- 
   - _семантическая карта текста_ -- бинарная матрица, высота которой кратна количеству обнаружеваемых атрибутов, а длинна соотв. кол-ву токенов в тексте.
...
## Некоторые подходы к обучению нейронной модели
### TL
- Мы полагаемся на предварительно обученные открытые модели третьих сторон, 
    применяются механизмы Transfer learning (TL).
### MTL
- Мы используем multi-task learning (MTL) подход к обучению, когда модель обучается одновременно классификации предмета контракта и определению атрибутов, которые косвенно связаны. Это улучшает сходимость модели и уменьшает оверфиттинг (overfitting).
### Обучающий набор   
3. Мы ограниченно применяем так называемый протокол обучения Noisy Student -- 
    ...
    предыдущей версией модели, в то время как большинство документов размечены вручную.
...
- К эмбеддингам  добавляется некоторое количество шума.
...
![Статистика по предметам договоров](training_reports/Distribution_of_subjects-ALL.png)
